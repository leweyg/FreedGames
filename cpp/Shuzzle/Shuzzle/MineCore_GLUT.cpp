...
ResMeshEdger* MyEdger=0;
void MyEvent_ContextFlushDone(void* data)
{
	if (MyEdger==0)
	{
		RenderContext* rc = Core.Graphics->GetRenderStage( SolidRenderContext );
		MeshRenderContext* mc = (MeshRenderContext*)rc;
		MyEdger = ResMeshEdger::Create( mc->Target );
		MyEdger->mRefPoint = Core.Graphics->CameraPos;
		mc->Target->mEdger = MyEdger;
		MyEdger->UpdateEdgeList();
		MyEdger->UpdateNormals(0, -1);
	}

	MyEdger->UpdateOutline();
	MyEdger->FlushOutline();
}

void Test_Main()
{
	LC_Event_DestroyCompNode = MyEvent_DestroyCompNode;
	LC_Event_ContextFlushDone = MyEvent_ContextFlushDone;
